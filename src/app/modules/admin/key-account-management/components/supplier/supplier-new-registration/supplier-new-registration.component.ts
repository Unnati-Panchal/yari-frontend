...
@Component({
  selector: 'app-supplier-new-registration',
  templateUrl: './supplier-new-registration.component.html',
  styleUrls: ['./supplier-new-registration.component.scss']
})
export class SupplierNewRegistrationComponent implements OnInit, OnDestroy {
  ...
  public registerSupplier(): void {
    ...
    if (!this.regForm.valid) {
      this.loading = false;
      Utilities.scrollToFirstInvalidControl();
      return;
    }
    ...
    regRequest.state = regRequest.state.name;
  ...
  public initRegistrationForm(): void {
    ...
    this._subscription.add(
      this.regForm.get('state').valueChanges.subscribe( (state) => {
        this._store.dispatch(fromProductsActions.getCities({stateId: state.id}));
        this.regForm.updateValueAndValidity();
      })
    );
  }
